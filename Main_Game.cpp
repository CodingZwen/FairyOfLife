...
Main_Game::Main_Game(sf::Vector2i screenDimensions,Game &g)
{
	...
	assetmanager.LoadFont("font", "Spiel_Projekt_Protectyourisle_Assets\\Fonts\\KenPixel Future Square.ttf");
	...
	gui->init(&assetmanager,&sh, Spieler1.get(),g.window,assetmanager.getfilemaker(),g);
...
void Main_Game::HandleEvents(Game & game)
{
	...
	while (game.window.pollEvent(e)) {
		...
		if (e.key.code == sf::Keyboard::Escape || gui->getOptionframe()->gomainmenue()) {
			...
			sf::View nview(sf::FloatRect(0.f, 0.f, static_cast<float>(game.window.getSize().x), static_cast<float>(game.window.getSize().y)));
...
{	
	...
	if (TileMapPainter.collide_engine(Spieler1->getPosition(),sf::Vector2f(0,0))) {
		...
		gameView.setCenter(sf::Vector2f(camerapos.x,camerapos.y));

		//als ich gui hier geupdatet habe war viewcenter immer standard 
		//da unten erst gesetzt ??
	}
	...
	//cout << "Spielery: " << Spieler1->getPosition().y << endl;
...
void Main_Game::Draw(Game & game)
{
	...
	eh.drawbehindplayer(game.window);//entitiys all
	Spieler1->draw(game.window);
	eh.drawinfrontplayer(game.window);//entitiys all
	...
	sf::View newview;
	newview.setViewport(sf::FloatRect(0.80f,0.05f, 0.32f, 0.32f));
	...
	//game.window.draw(TileMapPainter.getMinimap());
	game.window.draw(TileMapPainter.getMiniMapasCircle());
	

	newview.setViewport(sf::FloatRect(0, 0.f,1, 1));
	game.window.setView(newview);
...
void Main_Game::handle_menues(sf::Event e)
{
	if (e.type == sf::Event::KeyPressed) {
		...
		if (e.key.code == sf::Keyboard::F5) {
			...
			Gamesaver.createSaveFile("sven", Spieler1->getPosition().x, Spieler1->getPosition().y
				, Spieler1->attributs.get_level(), gui->getmoney(),
				Spieler1->attributs.getCurrentXP(), items.getInventoryItems());
		...
		if (e.key.code == sf::Keyboard::F6) {
			eh.killAllEntitys();
			items.clear_inventory();
			int posx, posy, level, money, xp;
			 std::string file = "newsavefile.xml";
			Gamesaver.loadSavefile(Spieler1->getname(), posx, posy
				,level,money,
				xp, items.getInventoryItems(), items.getItemSprites(), assetmanager.ptr_GetFont("font"),
				file);

			Spieler1->setplayerpos(sf::Vector2f(posx, posy));
			Spieler1->attributs.setLevel(level);
			Spieler1->attributs.setXP(xp);
			gui->give_money(money);
		...
		if (e.key.code == sf::Keyboard::F7) {
			...
			std::cout << "Inventory groesse: " << items.getInventoryItems().size();
...
{
	sf::Vector2f mouse_pos = game.window.mapPixelToCoords(mousepos);
	

	items.update_dropped_items(game.framedeltatime,sf::Vector2i(mouse_pos), Spieler1->movementComponent->getGlobalBounds(),Spieler1->attributs);
	...
	if (showinventory) {
		items.update_inventory(game.framedeltatime, sf::Vector2i(game.window.mapPixelToCoords(mousepos)),sf::Vector2f( camerapos), Spieler1->movementComponent->getGlobalBounds());
	}

	if (showequip) {
		items.update_equip(sf::Vector2i(game.window.mapPixelToCoords(mousepos)), sf::Vector2f(camerapos),Spieler1->movementComponent->getGlobalBounds());
	}
