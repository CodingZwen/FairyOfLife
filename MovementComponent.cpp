...
MovementComponent::MovementComponent(AnimatedSprite &animation, const float max_Velocity, float beschleunigung, float verzoegerung)
...
const bool MovementComponent::getStates(const short unsigned state)
{
	...
	case 5:
		...
		break;
	//case 6: //standing down 
	//	if (!(this->velocity.x + this->velocity.y) > 0 && lastState == 2) //wenn nichtb bewgt und letzter state hoch gehen war dann ist

	//		return true;
	//	break;
	//case 7: //standing links 
	//	if (!(this->velocity.x + this->velocity.y) > 0 && lastState ==3) //wenn nichtb bewgt und letzter state hoch gehen war dann ist

	//		return true;
	//	break;
	//case 8: //standing  rechts
	//	if (!(this->velocity.x + this->velocity.y) > 0 && lastState == 4) //wenn nichtb bewgt und letzter state hoch gehen war dann ist
...
{
	return ptrAnimation->getGlobalBounds();
}
...
int MovementComponent::returnDirection()
{
...
void MovementComponent::move(const sf::Vector2f velo, const float &dt)
{
...
void MovementComponent::moveWithoutAcceleration(const sf::Vector2f velo, const float & dt)
{
	/*this->velocity.x += velo.x;
	this->velocity.y += velo.y;*/
	this->ptrAnimation->move(velo*dt);
...
void MovementComponent::update(const float & dt)
{
	...
	if (!freezeVelo)
	{
		...
		if (velocityFreezetime > 0.10) {
			printf("velox: %f veloy: %f\n", getVelocity().x, getVelocity().y);

			freezeVelo = false;
			velocityFreezetime = 0;
		}
	}
	//final move
	...
	if (!isColliding)
	{
		...
		currentPosition = sf::Vector2f(ptrAnimation->getGlobalBounds().left, ptrAnimation->getGlobalBounds().top);
