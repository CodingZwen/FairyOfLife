...
private: 
	 ...
	 AnimatedSprite  *ptrAnimation = nullptr;
	 short lastState;
...
public:
	...
	MovementComponent(AnimatedSprite &animation,const float maxVelocity, float beschleunigung, float verzoegerung);
	...
	void move(const sf::Vector2f velo, const float &dt);
	void moveWithoutAcceleration(const sf::Vector2f velo, const float &dt);
